...
* [ ] Initialize Node.js/SvelteKit project and connect to the existing MySQL database.
* [ ] Implement an ORM (Prisma/Drizzle) and map existing tables (`users`, `flight`, `booking`, `passengers`, `payment`, `admin`).
* [ ] Create secure RESTful API endpoints for CRUD operations (e.g., `GET /api/flights`, `POST /api/bookings`).
* [ ] Implement JWT-based authentication for user and admin roles.

### Phase 2: Reactive Frontend Rebuild (Svelte 5)
* [ ] Setup SvelteKit with Tailwind CSS.
* [ ] Build reusable UI components (Flight Cards, Booking Forms, Navigation Modal).
* [ ] Develop the Interactive Dashboard consuming the new backend APIs.
* [ ] Implement client-side form validation and error handling for a seamless booking flow.

### Phase 3: Advanced Portfolio Features
* [ ] **Mobile Optimization:** Ensure fluid, responsive design across all breakpoints.
* [ ] **Flight Recommendations:** Build a logic module to suggest flights based on past booking history.
* [ ] **Automated PDF Generation:** Allow users to download their flight tickets or booking receipts as dynamically generated PDFs.
