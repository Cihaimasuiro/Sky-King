        ...
        <section id="reservation" class="section">
            ...
            <form method="POST" action="">
                ...
                <select id="customer_id" name="customer_id" required>
                    ...
                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            echo "<option value='{$row['id_customer']}'>{$row['id_customer']} - {$row['nama']}</option>";
                        }
                    }
                    ?>
                </select>
                ...
                <select id="flight_id" name="flight_id" required>
                    <?php
                    $sql = "SELECT id_penerbangan, nomor_penerbangan FROM Penerbangan";
                    $result = $conn->query($sql);

                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            echo "<option value='{$row['id_penerbangan']}'>{$row['id_penerbangan']} - {$row['nomor_penerbangan']}</option>";
                        }
                    }
                    ?>
                </select>
            ...
            if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['reserve'])) {
                ...
                $sql = "INSERT INTO Pemesanan (tanggal_pesan, status, id_customer, id_penerbangan) VALUES (?, ?, ?, ?)";
                $stmt = $conn->prepare($sql);
                $stmt->bind_param('ssii', $booking_date, $status, $customer_id, $flight_id);
